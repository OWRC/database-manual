...
,cast(moeh.HOLE_DIAMETER_UOM collate database_default as varchar(50)) as BH_DIAMETER_UNIT_OUOM
,cast(moecwt.[des] collate database_default as varchar(255)) as MOE_BH_GEOLOGY_CLASS
...
left outer join MOE_20250711.dbo.YC_20250711_DRILL_CODE as ydc
...
from 
MOE_20250711.dbo.TblHole as moeh
where
...
left outer join MOE_20250711.dbo.YC_20250711_FINAL_STATUS as ycfs
on moewwr.FINAL_STA collate database_default = ycfs.FINAL_STA
inner join MOE_20250711.dbo.TblBore_Hole as moebh
on y.BORE_HOLE_ID=moebh.BORE_HOLE_ID
left outer join MOE_20250711.dbo._code_Well_Type as moecwt
on moebh.CODEOB collate database_default = moecwt.code collate database_default
...
,rtrim( cast( ydc.BH_COMMENT as varchar(255) ) ) as BH_COMMENT
,row_number() over (order by y.bore_hole_id) as rkey
into MOE_20250711.dbo.M_D_BOREHOLE
from 
MOE_20250711.dbo.YC_20250711_BH_ID as y
left outer join MOE_20250711.dbo.YC_20250711_DRILL_CODE as ydc
on y.BORE_HOLE_ID=ydc.BORE_HOLE_ID
...
-- v20230324 19504 rows
-- v20240326 24032 rows
-- v20250711 18973 rows

select
count(*)
from 
moe_20250711.dbo.m_d_borehole
...
select
*
into moe_20250711.dbo.M_D_BOREHOLE_bck
from 
moe_20250711.dbo.M_D_BOREHOLE
...
-- v20230324 repeated three times
-- v20240326 repeated twice
-- v20250711 repeated twice
...
,dbore.rkey
--into MOE_20250711.dbo.YC_20250711_DBORE_DRILL_CODES_1
--into MOE_20250711.dbo.YC_20250711_DBORE_DRILL_CODES_2
--into MOE_20250711.dbo.YC_20250711_DBORE_DRILL_CODES_3
--into MOE_20250711.dbo.YC_20250711_DBORE_DRILL_CODES_4
from 
MOE_20250711.dbo.M_D_BOREHOLE as dbore
inner join MOE_20250711.dbo.V_DBORE_DRILL_CODES as v
on dbore.rkey=v.rkey

delete from MOE_20250711.dbo.M_D_BOREHOLE
where rkey in
( 
select
rkey
from 
--MOE_20250711.dbo.YC_20250711_DBORE_DRILL_CODES_1
MOE_20250711.dbo.YC_20250711_DBORE_DRILL_CODES_2
--MOE_20250711.dbo.YC_20250711_DBORE_DRILL_CODES_3
--MOE_20250711.dbo.YC_20250711_DBORE_DRILL_CODES_4
)

-- the following is used if an error was made, above

--drop table MOE_20250711.dbo.YC_20250711_DBORE_DRILL_CODES_1

--drop table MOE_20250711.dbo.YC_20250711_DBORE_DRILL_CODES_2

--drop table MOE_20250711.dbo.YC_20250711_DBORE_DRILL_CODES_3

--drop table MOE_20250711.dbo.M_D_BOREHOLE

--select
--*
--into MOE_20250711.dbo.M_D_BOREHOLE
--from 
--MOE_20250711.dbo.M_D_BOREHOLE_bck
...
-- v20230324 18826 rows - okay
-- v20240326 23131 rows - okay
...
--delete from MOE_20250711.dbo.YC_20250711_DBORE_DRILL_CODES_3
--where loc_id in
--(
--select
--y.loc_id
--from 
--MOE_20250711.dbo.M_D_BOREHOLE as m
--inner join MOE_20250711.dbo.YC_20250711_DBORE_DRILL_CODES_3 as y
--on m.loc_id=y.loc_id
--where
--m.bh_drill_method_code=y.bh_drill_method_code
)

delete from MOE_20250711.dbo.YC_20250711_DBORE_DRILL_CODES_2
where loc_id in
(
select
y.loc_id
from 
MOE_20250711.dbo.M_D_BOREHOLE as m
inner join MOE_20250711.dbo.YC_20250711_DBORE_DRILL_CODES_2 as y
on m.loc_id=y.loc_id
where
m.bh_drill_method_code=y.bh_drill_method_code
)

delete from MOE_20250711.dbo.YC_20250711_DBORE_DRILL_CODES_1
where loc_id in
(
select
y.loc_id
from 
MOE_20250711.dbo.M_D_BOREHOLE as m
inner join MOE_20250711.dbo.YC_20250711_DBORE_DRILL_CODES_1 as y
on m.loc_id=y.loc_id
where
m.bh_drill_method_code=y.bh_drill_method_code
)
...
y2.loc_id
from 
MOE_20250711.dbo.YC_20250711_DBORE_DRILL_CODES_1 as y1
inner join MOE_20250711.dbo.YC_20250711_DBORE_DRILL_CODES_2 as y2
...
--y2.loc_id
--from 
--MOE_20250711.dbo.YC_20250711_DBORE_DRILL_CODES_2 as y1
--inner join MOE_20250711.dbo.YC_20250711_DBORE_DRILL_CODES_3 as y2
--on y1.loc_id=y2.loc_id
--where
--y1.bh_drill_method_code=y2.bh_drill_method_code
--)
...
-- make a backup copy of M_D_BOREHOLE (this includes the updates to the drilling codes)

select
*
into moe_20250711.dbo.M_D_BOREHOLE_bck2
from 
moe_20250711.dbo.M_D_BOREHOLE
...
from 
MOE_20250711.dbo.M_D_BOREHOLE as moe
inner join
(
select
dbore.LOC_ID
...
when d2.loc_id is not null then
	'; ' + r2.bh_drill_method_description -- use this (only) if there are only two iterations; the following if three
else ''
end as to_add
from 
MOE_20250711.dbo.M_D_BOREHOLE as dbore
inner join MOE_20250711.dbo.YC_20250711_DBORE_DRILL_CODES_1 as d1
...
left outer join MOE_20250711.dbo.YC_20250711_DBORE_DRILL_CODES_2 as d2
...
left outer join OAK_20160831_MASTER.dbo.R_BH_DRILL_METHOD_CODE as r2
on d2.bh_drill_method_code=r2.bh_drill_method_code
left outer join MOE_20250711.dbo.YC_20250711_DBORE_DRILL_CODES_3 as d3
...
-- v20240326 23131 records
-- v20250711 18379 records
...
-- if there is an issue, delete the M_D_BOREHOLE table and copy the backup
